...
def verify_face(
    auth_image: np.ndarray, target_user: Optional[str] = None
    ...
    for user_id in users:
        ...
        # Compare features
        print(
            f"- Auth features shape: {auth_features.shape if auth_features is not None else None}"
        )
        print(
            f"- Model features shape: {model_features.shape if model_features is not None else None}"
        )
